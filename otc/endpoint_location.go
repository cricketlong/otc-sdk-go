...
// need to specify a Name and/or a Region depending on what's available on your OpenStack
// deployment.
func V3EndpointURL(catalog *tokens.ServiceCatalog, opts opentelekomcloud.EndpointOpts) (string, error) {
	// Extract Endpoints from the catalog entries that match the requested Type, Interface,
	// Name if provided, and Region if provided.
	var endpoints = make([]tokens.Endpoint, 0, 1)
	for _, entry := range catalog.Entries {
		if (entry.Type == opts.Type) && (opts.Name == "" || entry.Name == opts.Name) {
			for _, endpoint := range entry.Endpoints {
				if opts.Availability != opentelekomcloud.AvailabilityAdmin &&
					opts.Availability != opentelekomcloud.AvailabilityPublic &&
					opts.Availability != opentelekomcloud.AvailabilityInternal {
					err := &ErrInvalidAvailabilityProvided{}
					err.Argument = "Availability"
					err.Value = opts.Availability
					return "", err
				}
				if (opts.Availability == opentelekomcloud.Availability(endpoint.Interface)) &&
	...
	for _, endpoint := range endpoints {
		return opentelekomcloud.NormalizeURL(endpoint.URL), nil
	}

	// Report an error if there were no matching endpoints.
	err := &opentelekomcloud.ErrEndpointNotFound{}
	return "", err
}
