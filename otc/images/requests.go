...
import (
	...
	"github.com/cricketlong/otc-sdk-go/pagination"
)
...
func (opts ListOpts) ToImageListQuery() (string, error) {
	q, err := opentelekomcloud.BuildQueryString(opts)
	return q.String(), err
}

// ListDetail enumerates the available images.
func ListDetail(client *opentelekomcloud.ServiceClient, opts ListOptsBuilder) pagination.Pager {
...
func Get(client *opentelekomcloud.ServiceClient, id string) (r GetResult) {
	_, r.Err = client.Get(getURL(client, id), &r.Body, nil)
	return
}

// Delete deletes the specified image ID.
func Delete(client *opentelekomcloud.ServiceClient, id string) (r DeleteResult) {
	_, r.Err = client.Delete(deleteURL(client, id), nil)
	return
}

// IDFromName is a convienience function that returns an image's ID given its name.
func IDFromName(client *opentelekomcloud.ServiceClient, name string) (string, error) {
	count := 0
	id := ""
	...
	switch count {
	case 0:
		err := &opentelekomcloud.ErrResourceNotFound{}
	...
	case 1:
		return id, nil
	default:
		err := &opentelekomcloud.ErrMultipleResourcesFound{}
		err.ResourceType = "image"
		err.Name = name
		err.Count = count
		return "", err
