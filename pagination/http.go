...
func PageResultFromParsed(resp *http.Response, body interface{}) PageResult {
	return PageResult{
		Result: opentelekomcloud.Result{
			Body:   body,
			Header: resp.Header,
		},
...
func Request(client *opentelekomcloud.ServiceClient, headers map[string]string, url string) (*http.Response, error) {
	return client.Get(url, nil, &opentelekomcloud.RequestOpts{
		MoreHeaders: headers,
		OkCodes:     []int{200, 204},
	})
}
